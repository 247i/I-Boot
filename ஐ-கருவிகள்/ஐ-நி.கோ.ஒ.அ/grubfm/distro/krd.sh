...
# set initrd_img="/boot/grub/initrd.xz";
# if cpuid -l;
# then
...
# menuentry $"Kaspersky Rescue Disk. Graphic mode (${kernel_postfix})" --class $icon{
    # linux /boot/grub/k-${kernel_postfix} $kcmdline $linux_extra dostartx;
    # initrd (md_initrd);
# }

# menuentry $"Kaspersky Rescue Disk. Limited graphic mode (${kernel_postfix})" --class $icon{
    # linux /boot/grub/k-${kernel_postfix} $kcmdline $linux_extra dostartx nomodeset;
    # initrd (md_initrd);
# }

# menuentry $"Kaspersky Rescue Disk. Text mode (${kernel_postfix})" --class $icon{
    # linux /boot/grub/k-${kernel_postfix} $kcmdline $linux_extra nox nomodeset;
    # initrd (md_initrd);
# }

# menuentry $"Kaspersky Rescue Disk. Hardware Info (${kernel_postfix})" --class $icon{
    # linux /boot/grub/k-${kernel_postfix} $kcmdline $linux_extra docache loadsrm=000-core.srm,003-kl.srm nox hwinfo docheck;
    # initrd (md_initrd);
# }
