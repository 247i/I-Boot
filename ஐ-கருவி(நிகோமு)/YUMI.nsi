...
; Wipes the Disk # and reformats it with two partitions (1st exFAT and 2nd fat)
	...
	; todo call and use a new DrivesListBoot function... only show drives with the TEMPYUMI label that match disk
   ${GetDrives} "FDD+HDD" DrivesListBoot ; probe for the BOOT disk number and letter... since we just wiped and recreated partitions on the disk.
	...
	; this finalizes the BOOT partition - after we have added our boot files.
	${If} $BOOT_LETTER != ""
...
; Make sure EFI\Boot directory and files exist.  
  ...
  # Use the following as a backup if the NSIS 7z dll breaks
    ...
    nsExec::ExecToLog '"$PLUGINSDIR\grub-install.exe" --force --removable --no-floppy --target=i386-pc --boot-directory=$BOOT_LETTER\boot //./PHYSICALDRIVE$INST_DISK' 
  ${EndIf}    
FunctionEnd
